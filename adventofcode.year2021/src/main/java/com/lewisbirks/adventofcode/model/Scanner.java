...
public record Scanner(String name, List<Point3D> coordinates) {
    ...
    public Point3D findTranslationPoint(Scanner other) {
        ...
        coordinates.forEach(
            coordinate -> other.coordinates.stream().map(coordinate::subtract).forEach(pointFrequencies::put)
        );

        List<Map.Entry<Point3D, Long>> reoccurringPoints =
            pointFrequencies.entriesMatching(e -> e.getValue() >= 12);
